...
//neko
var neko={timers:[],timerData:{}};$(function(){neko.init()}),neko.init=function(){$("#reminder-submit").on("click",neko.submit),"default"===Notification.permission&&Notification.requestPermission(),neko.ticker=new interval(neko.tick,100),window.addEventListener("blur",function(){$("#reminder-onblur-setting").prop("checked")&&neko.ticker.running&&(neko.ticker.end(),neko.ticker.time=5e3,neko.ticker.start())}),window.addEventListener("focus",function(){if(100!=neko.ticker.time){let a=neko.ticker.running;a&&neko.ticker.end(),neko.ticker.time=100,a&&neko.ticker.start()}}),$("#timer-area").sortable({containment:"#timer-area"}),neko.loadStorage()},neko.submit=function(){let a=tagsInputRead($("#reminder-number")[0]),b=0,c=$("#reminder-note").val().replace("<","&lt;").replace(">","&gt;");if(!(1>a.length)){for(let c in a)/^([0-9]+)(h|hr|m|min|s|sec)$/g.test(a[c])?b+=interval.prototype.convert(a[c]):/^[0-9]+$/g.test(a[c])&&(b+=interval.prototype.convert(a[c]+"s"));0==b||(b=0|b/1e3,neko.createTimer(b,c))}},neko.createTimer=function(a,b,c){let d,e,f={},g=c||Date.now();d=new Date(g+1e3*a),f.target=g+1e3*a,f.note=b||"Untitled Timer",f.lastTick=g,f.startedAt=g,f.id=g.toString(16),f.status="running",e=$("<div>",{id:f.id,class:"timer"}),e.append($("<button>").html("X").on("click",function(){let a=neko.timerData[f.id],b=$("#"+a.id);if("running"==a.status){if(0<b.find(".confirm-box").length)return;let c=$("<div>",{class:"confirm-box"});return c.html("Do you really want to delete &quot;"+a.note+"&quot;?<br><br>").append($("<button>").html("Yes").on("click",function(){b.remove(),toastr.success("Removed Timer &quot;"+a.note+"&quot;","Removed Timer"),neko.timers.splice(neko.timers.indexOf(a.id),1)}.bind(a,b))).append($("<button>").html("No").on("click",function(){$(this).parent().remove()})),void b.append(c)}b.remove(),toastr.success("Removed Timer &quot;"+a.note+"&quot;","Removed Timer"),neko.timers.splice(neko.timers.indexOf(f.id),1)}.bind(f))),e.append($("<h2>").html(f.note)),e.append($("<div>",{class:"progress"}).append($("<div>",{class:"progress-value"}).css("width","0%").html("0%"))),e.append($("<span>",{class:"small"}).html("Ends in <span>"+neko.timeFromMilliseconds(1e3*a)+"</span> (at "+d.toLocaleTimeString()+")")),$("#timer-area").append(e),neko.timers.push(f.id),neko.timerData[f.id]=f,neko.ticker.start(),c===void 0?toastr.success("Created Timer &quot;"+f.note+"&quot;","Created Timer"):toastr.success("Loaded Timer &quot;"+f.note+"&quot; from cookies.","Loaded Timer"),neko.updateStorage()},neko.tick=function(){if(1>neko.timers.length)return void neko.ticker.end();for(let a of neko.timers){let b=neko.timerData[a];if("running"!=b.status)continue;let c=Date.now(),d=100*((c-b.startedAt)/(b.target-b.startedAt)),e=$("#"+a).find(".progress-value"),f=c-b.lastTick;if(b.lastTick=c,$("#"+a).find("span.small > span").html(neko.timeFromMilliseconds(b.target-c)),100<=d)return e.css("width","100%").html("100%").addClass("progress-finished"),b.status="finished",new Notification("Your Timer \""+b.note+"\" ended!"),void toastr.info("Timer &quot;"+b.note+"&quot; ended","Timer ended");e.css("width",d.toString()+"%").html((0|d).toString()+"%")}neko.updateStorage()},neko.timeFromMilliseconds=function(a){let b=Math.ceil(+a/1e3),c=!1,d="";if(0>b&&(b=0),3600<=b){let a=0|b/3600;b-=3600*a,d+=(10>a?"0":"")+a.toString()+":",c=!0}if(c||60<=b){let a=0|b/60;b-=60*a,d+=(10>a?"0":"")+a.toString()+":"}else d+="00:";return d+=(10>b?"0":"")+b.toString(),d},neko.updateStorage=function(){$("#reminder-allow-cookie")[0].checked?(localStorage.setItem("cookieEnabled","enabled"),localStorage.setItem("blurModeEnabled",$("#reminder-onblur-setting")[0].checked?"enabled":"disabled"),localStorage.setItem("timerData",JSON.stringify(neko.timerData)),localStorage.setItem("timers",JSON.stringify(neko.timers))):localStorage.clear()},neko.loadStorage=function(){if(null===localStorage.getItem("cookieEnabled"))return!1;$("#reminder-allow-cookie").prop("checked",!0),"enabled"===localStorage.getItem("blurModeEnabled")&&$("#reminder-onblur-setting").prop("checked",!0);let a=JSON.parse(localStorage.getItem("timerData")),b=JSON.parse(localStorage.getItem("timers"));neko.ticker.end();for(let c of b){let b=a[c];neko.createTimer(b.target-b.startedAt,b.note,b.startedAt),neko.timerData[b.id]=b}0<b.length&&neko.ticker.start()},toastr.options={closeButton:!0,debug:!1,newestOnTop:!0,progressBar:!0,positionClass:"toast-top-right",preventDuplicates:!1,onclick:null,showDuration:"300",hideDuration:"1000",timeOut:"5000",extendedTimeOut:"1000",showEasing:"swing",hideEasing:"linear",showMethod:"fadeIn",hideMethod:"fadeOut"};