...
//neko
var timerArea,reminderNum,reminderNote,reminderSubmit,timers=[],neko={};$(function(){neko.init()}),neko.init=function(){timerArea=$("#timer-area"),reminderNum=$("#reminder-number"),reminderNote=$("#reminder-note"),reminderSubmit=$("#reminder-submit"),reminderSubmit.on("click",neko.submit),"default"===Notification.permission&&Notification.requestPermission(),neko.ticker=new interval(neko.tick,100),window.addEventListener("blur",function(){$("#reminder-onblur-setting").prop("checked")&&neko.ticker.running&&(neko.ticker.end(),neko.ticker.time=5e3,neko.ticker.start())}),window.addEventListener("focus",function(){if(100!=neko.ticker.time){let a=neko.ticker.running;a&&neko.ticker.end(),neko.ticker.time=100,a&&neko.ticker.start()}}),timerArea.sortable({containment:"#timer-area"})},neko.submit=function(){let a,b,c={},d=Date.now(),e=tagsInputRead(reminderNum[0]),f=0;if(!(1>e.length)){for(let a in e)/^([0-9]+)(h|hr|m|min|s|sec)$/g.test(e[a])?f+=interval.prototype.convert(e[a]):/^[0-9]+$/g.test(e[a])&&(f+=interval.prototype.convert(e[a]+"s"));0==f||(f=0|f/1e3,a=new Date(d+1e3*f),c.target=d+1e3*f,c.note=reminderNote.val().replace("<","&lt;").replace(">","&gt;")||"Untitled Timer",c.lastTick=d,c.startedAt=d,c.id=d.toString(16),c.status="running",b=$("<div>",{id:c.id,class:"timer"}),b.append($("<button>").html("X").on("click",function(){let a=timers.findIndex(function(a){return a.id===c.id}.bind(c)),b=timers[a],d=$("#"+b.id);if("running"==b.status){if(0<d.find(".confirm-box").length)return;let c=$("<div>",{class:"confirm-box"});return c.html("Do you really want to delete &quot;"+b.note+"&quot;?<br><br>").append($("<button>").html("Yes").on("click",function(){d.remove(),toastr.success("Removed Timer &quot;"+timers[a].note+"&quot;","Removed Timer"),timers.splice(a,1)}.bind(a,d))).append($("<button>").html("No").on("click",function(){$(this).parent().remove()})),void d.append(c)}d.remove(),toastr.success("Removed Timer &quot;"+timers[a].note+"&quot;","Removed Timer"),timers.splice(a,1)}.bind(c))),b.append($("<h2>").html(c.note)),b.append($("<div>",{class:"progress"}).append($("<div>",{class:"progress-value"}).css("width","0%").html("0%"))),b.append($("<span>",{class:"small"}).html("Ends in <span>"+neko.timeFromMilliseconds(1e3*f)+"</span> (at "+a.toLocaleTimeString()+")")),timerArea.append(b),timers.push(c),neko.ticker.start(),toastr.success("Created Timer &quot;"+c.note+"&quot;","Created Timer"))}},neko.tick=function(){if(1>timers.length)return void neko.ticker.end();for(timer of timers){if("running"!=timer.status)continue;let a=Date.now(),b=100*((a-timer.startedAt)/(timer.target-timer.startedAt)),c=$("#"+timer.id).find(".progress-value"),d=a-timer.lastTick;if(timer.lastTick=a,$("#"+timer.id).find("span.small > span").html(neko.timeFromMilliseconds(timer.target-a)),100<=b)return c.css("width","100%").html("100%").addClass("progress-finished"),timer.status="finished",new Notification("Your Timer \""+timer.note+"\" ended!"),void toastr.info("Timer &quot;"+timer.note+"&quot; ended","Timer ended");c.css("width",b.toString()+"%").html((0|b).toString()+"%")}},neko.timeFromMilliseconds=function(a){let b=Math.ceil(+a/1e3),c=!1,d="";if(0>b&&(b=0),3600<=b){let a=0|b/3600;b-=3600*a,d+=(10>a?"0":"")+a.toString()+":",c=!0}if(c||60<=b){let a=0|b/60;b-=60*a,d+=(10>a?"0":"")+a.toString()+":"}else d+="00:";return d+=(10>b?"0":"")+b.toString(),d},toastr.options={closeButton:!0,debug:!1,newestOnTop:!0,progressBar:!0,positionClass:"toast-top-right",preventDuplicates:!1,onclick:null,showDuration:"300",hideDuration:"1000",timeOut:"5000",extendedTimeOut:"1000",showEasing:"swing",hideEasing:"linear",showMethod:"fadeIn",hideMethod:"fadeOut"};